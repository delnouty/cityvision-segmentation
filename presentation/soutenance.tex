...
% ---- Couleurs exactes des 9 classes (cf. cityvision/constants.py) ----
\definecolor{clsbg}{RGB}{0,0,0}       % background
\definecolor{clsveg}{RGB}{107,142,35} % vegetation
\definecolor{clscar}{RGB}{0,0,142}    % car
\definecolor{clssign}{RGB}{220,220,0} % traffic_sign
\definecolor{clsbike}{RGB}{119,11,32} % bicycle
  ...
  \begin{columns}[T]
    \begin{column}{0.58\textwidth}
      ...
      \begin{itemize}
        ...
        \item Simplification à \emphc{8 classes} utiles :\\
          \footnotesize \code{road, building, vegetation, sky,
          person, car, traffic\_sign, bicycle}\\
          \code{+ background} (tout le reste).
      \end{itemize}
    \end{column}
    \begin{column}{0.40\textwidth}
      \centering
      \begin{tikzpicture}[scale=0.62]
        % legende des 9 classes
        \foreach \name/\c/\y in {%
          background/clsbg/4.0, route/cvroad/3.5, bâtiment/cvgray/3.0,
          végétation/clsveg/2.5, ciel/cvsky/2.0, personne/cvred/1.5,
          voiture/clscar/1.0, panneau/clssign/0.5, vélo/clsbike/0.0}{
          \fill[\c] (0,\y) rectangle (0.4,\y+0.35);
          \draw[gray!40,line width=0.2pt] (0,\y) rectangle (0.4,\y+0.35);
...
%==============================================================
\begin{frame}{2 \textbar{} Les 4 architectures comparées}
  \centering
...
\begin{frame}{3 \textbar{} Architecture : API + application Web}
  ...
  \begin{tikzpicture}[
    ...
    \draw[{Stealth}-,very thick,cvgreen] ([yshift=-6pt]front.east) -- node[below,align=center,font=\scriptsize]{PNG : masque prédit\\JSON : classes détectées} ([yshift=-6pt]back.west);
    \coordinate (mid) at ($(front.south)!0.5!(back.south)$);
    \node[shared,below=1.1cm of mid] (core)
...
%==============================================================
%  ANNEXES (backup slides — hors plan principal)
%==============================================================
\appendix

\begin{frame}{Annexe \textbar{} Motivation de l'architecture}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \textbf{Découplage frontend / backend}
      \begin{itemize}\footnotesize
        \item Frontend \emphc{Streamlit léger} (sans PyTorch) $\leftrightarrow$
          backend \emphc{FastAPI} qui embarque le modèle.
        \item Déploiement \& montée en charge \emphc{indépendants} ;
          le frontend n'a aucune dépendance lourde.
        \item Contrat clair : le client ne fait que des appels HTTP.
      \end{itemize}
      \vspace{0.3em}
      \textbf{Pourquoi \emphc{FastAPI}}
      \begin{itemize}\footnotesize
        \item Inférence CPU en \emphc{threadpool} (boucle async non bloquée).
        \item Validation d'entrée, \emphc{OpenAPI} auto (\code{/docs}),
          garde-fous (limite d'upload).
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
      \textbf{Une API \emphc{« style REST »}}
      \begin{itemize}\footnotesize
        \item \emphc{Sans état} : chaque requête est autonome, modèle chargé
          une fois au démarrage.
        \item Méthodes HTTP standard (\code{GET /health}, \code{POST /predict}\dots),
          codes de statut (413, 503), représentations \emphc{JSON \& PNG}.
        \item \emphc{Interopérable} : tout client (curl, Streamlit, navigateur).
      \end{itemize}
      \vspace{0.3em}
      \textbf{Nuance honnête : REST \emph{pragmatique}}
      \begin{itemize}\footnotesize
        \item Points d'entrée \emphc{orientés action} (\code{/predict},
          \code{/predict/image}) plutôt qu'orientés ressource
          (\code{/predictions/\{id\}}).
        \item Pas de \emph{HATEOAS}, pas de ressource adressable persistante
          --- inutile pour une inférence sans état.
        \item Compromis assumé : \emphc{simplicité} $>$ pureté REST pour un
          service d'inférence.
      \end{itemize}
    \end{column}
  \end{columns}
...
\begin{frame}{Annexe \textbar{} Motivation du choix Cloud}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \textbf{Critères de sélection}
      \begin{itemize}\footnotesize
        \item Démo \emphc{publique}, simple et à \emphc{faible coût}.
        \item Déploiement \emphc{reproductible} (Docker) et rapide
          (\code{git push}).
        \item Écosystème orienté \emphc{ML / IA}.
      \end{itemize}
      \vspace{0.3em}
      \textbf{Pourquoi \emphc{Hugging Face Spaces}}
      \begin{itemize}\footnotesize
        \item \emphc{SDK Docker natif} : la même image qu'en local, pas de refonte.
        \item Déploiement par \code{git push} ; URL publique + HTTPS fournis.
        \item \emphc{Deux Spaces} : API (FastAPI) + app (Streamlit),
          reliés par \code{CITYVISION\_API}.
        \item API en \emphc{always-on} (payant) pour éviter les \emph{cold starts} ;
          app sur l'offre gratuite.
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
      \textbf{Alternatives considérées}
      \begin{itemize}\footnotesize
        \item \emphc{AWS / GCP / Azure} : plus puissants (GPU, autoscaling)
          mais configuration \& coût plus lourds pour une simple démo.
        \item \emphc{Render / Railway} : proches en simplicité, mais moins
          orientés ML et sans écosystème de modèles.
      \end{itemize}
      \vspace{0.3em}
      \textbf{Compromis assumés}
      \begin{itemize}\footnotesize
        \item \emphc{CPU only} : latence acceptable pour la démo
          (GPU si passage à l'échelle).
        \item Dépendance à HF, mais l'\emphc{image Docker est portable}
          $\rightarrow$ migration possible sans réécriture.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
